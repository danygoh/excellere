// Validator Review Page
export const dynamic = 'force-dynamic';

export default async function ValidatorReviewPage({ params }) {
  const reportId = params.id || 'r1';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 380px', gap: '20px' }}>
          ...
          <div style={{ background: '#0a0a0a', border: '1px solid #222', padding: '30px' }}>
            ...
            <form action="/validate" method="GET">
              ...
              <textarea name="comment" required placeholder="Write 2-4 sentences about this learner's progress..." style={{ width: '100%', height: '140px', background: '#1a1a1a', border: '1px solid #333', color: '#fff', padding: '12px', fontSize: '13px', resize: 'none', marginBottom: '20px' }} />
              
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px', color: '#888', fontSize: '13px' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', color: '#888', fontSize: '13px' }}>
                <input type="radio" name="status" value="exceptional" style={{ accentColor: '#d4af37' }} />
                ★ Exceptional
              </label>
              
              <button type="submit" style={{ width: '100%', background: '#d4af37', color: '#000', border: 'none', padding: '14px', fontSize: '12px', fontWeight: 600, letterSpacing: '1px', cursor: 'pointer' }}>
                SIGN & ISSUE
              </button>
            </form>
          </div>
